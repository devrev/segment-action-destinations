...
const action: ActionDefinition<Settings, Payload> = {
  ...
  description:
    'Creates a Contact, unless one already exists with the same email address. If provided, will also tag and comment on the Contact',
  fields: {
    email: {
      label: 'Email',
    ...
    owner: {
      label: 'Owner',
      description: 'Owner to assign to newly created accounts or contacts',
      type: 'string',
      required: false,
      dynamic: true
    },
    ownerFieldName: {
      label: 'Contact owner field name',
      description: 'The custom field to use for Contact owners in DevRev',
      type: 'string',
      required: false,
      default: 'owner'
    },
  ...
  dynamicFields: {
    owner: async (request, { settings }): Promise<DynamicFieldResponse> => {
      try {
        const result: DevUserListResponse = await request(`${getBaseUrl(settings)}${devrevApiPaths.devUsersList}`, {
          method: 'get',
          skipResponseCloning: true
        })
        const choices = result.data.dev_users.map((devu) => {
          return { value: devu.id, label: `${devu.display_name} ${devu.email}` }
        })
        return { choices, nextPage: result.data.next_cursor }
      } catch (err) {
        return {
          choices: [],
          nextPage: '',
  ...
  perform: async (request, { settings, payload }) => {
    ...
    if (existingUsers.data.rev_users.length == 0) {
      ...
      if (existingAccount.data.accounts.length > 0) {
        ...
        if (payload.owner) createAccountPayload.owned_by = [payload.owner]
      ...
      // If there is an owner, assign the new contact
      if (payload.owner && payload.ownerFieldName)
        createUserPayload.custom_fields = { [`tnt__${payload.ownerFieldName}`]: payload.owner }
